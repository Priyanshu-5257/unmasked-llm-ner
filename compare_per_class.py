...
def get_per_class_results(predictions, labels):
...
def roberta_tokenize_and_align_labels(examples):
    tokenized_inputs = roberta_tokenizer(
        examples["tokens"],
        truncation=True,
        is_split_into_words=True,
        max_length=128
...
roberta_tokenized = dataset.map(roberta_tokenize_and_align_labels, batched=True)

roberta_model = AutoModelForTokenClassification.from_pretrained("roberta-base", num_labels=num_labels)
sd = load_file("./results_roberta/checkpoint-2634/model.safetensors")
roberta_model.load_state_dict(sd)
roberta_model.eval()
roberta_model.cuda()

roberta_trainer = Trainer(
    model=roberta_model,
    eval_dataset=roberta_tokenized["test"],
    processing_class=roberta_tokenizer,
    data_collator=DataCollatorForTokenClassification(roberta_tokenizer),
)

roberta_predictions, roberta_labels, _ = roberta_trainer.predict(roberta_tokenized["test"])
roberta_results = get_per_class_results(roberta_predictions, roberta_labels)

print("\nRoBERTa Per-class F1:")
for entity in ["PER", "ORG", "LOC", "MISC"]:
    f1 = roberta_results.get(f"{entity}", {}).get("f1", 0)
    prec = roberta_results.get(f"{entity}", {}).get("precision", 0)
    rec = roberta_results.get(f"{entity}", {}).get("recall", 0)
    print(f"  {entity}: P={prec:.4f}, R={rec:.4f}, F1={f1:.4f}")
print(f"  Overall: F1={roberta_results['overall_f1']:.4f}")

print("\n" + "=" * 60)
print("Evaluating LFM Unmasked (8 Layers + LoRA) on test set...")
print("=" * 60)

lfm_model = LFMForTokenClassification(model_name, num_labels)
lfm_model.load_state_dict(load_file("./results_lfm_unmasked_lora_8layers/checkpoint-2634/model.safetensors"))
lfm_model.eval()
lfm_model.cuda()
...
lfm_predictions, lfm_labels, _ = lfm_trainer.predict(tokenized_datasets["test"])
lfm_results = get_per_class_results(lfm_predictions, lfm_labels)

print("\nLFM Per-class F1:")
for entity in ["PER", "ORG", "LOC", "MISC"]:
    f1 = lfm_results.get(f"{entity}", {}).get("f1", 0)
    prec = lfm_results.get(f"{entity}", {}).get("precision", 0)
    rec = lfm_results.get(f"{entity}", {}).get("recall", 0)
    print(f"  {entity}: P={prec:.4f}, R={rec:.4f}, F1={f1:.4f}")
print(f"  Overall: F1={lfm_results['overall_f1']:.4f}")
