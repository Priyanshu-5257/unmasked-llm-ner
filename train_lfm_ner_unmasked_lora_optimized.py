...
class LFMForTokenClassification(nn.Module):
    def __init__(self, model_name, num_labels):
        ...
        # Patch the conv layers in the last 12 layers if they exist
        ...
        # We target the attention projections and MLP layers.
        # Conv layers have 'in_proj' and 'out_proj', while Attention has 'q_proj', 'k_proj', 'v_proj', 'o_proj'
        # Let's target all linear-like modules to be safe or specify them explicitly.
        ...
        lora_config = LoraConfig(
            ...
            lora_dropout=0.1,
            bias="none",
            modules_to_save=[], # We don't save the base model embeddings, just the lora weights
        )
        ...
        # NOTE: PEFT automatically freezes the base model parameters that aren't part of LoRA adapters.
        # Ensure the classifier requires gradients
...
# Check if the patch survived PEFT wrapping
inner = model.base_model.model  # the actual Lfm2Model inside PeftModel
print(inner.forward.__name__)  # Should print "patched_lfm_model_forward"
# If it prints "forward", your patch was clobbered

# Also check a conv layer in the last 12
layer_idx = model.base_model.config.num_hidden_layers - 12
print(model.base_model.model.layers[layer_idx].conv.forward.__name__)
...
training_args = TrainingArguments(
    output_dir="./results_lfm_unmasked_lora_optimized",
    eval_strategy="epoch",
    learning_rate=2e-4,
...
print("Starting training for Custom LFM NER (Unmasked Last 12 Layers + LoRA r=64)...")
# trainer.train()
## load model from checkpoint
trainer.train(resume_from_checkpoint="results_lfm_unmasked_lora_optimized/checkpoint-3512")
# Evaluate on test set
print("Evaluating on test set...")
test_results = trainer.evaluate(tokenized_datasets["test"])
print(test_results)

with open("lfm_unmasked_lora_optimized_test_results.json", "w") as f:
