...
def patched_lfm_model_forward(
    ...
    **kwargs,
    ...
    # Middle 8 layers: layers 4-11 (for 16-layer model)
    mid_start = num_layers // 2 - 4
    mid_end = num_layers // 2 + 4
    for i, decoder_layer in enumerate(self.layers[:num_layers]):
        # UNMASK MIDDLE 8 LAYERS
...
class LFMForTokenClassification(nn.Module):
    def __init__(self, model_name, num_labels):
        ...
        # Patch the conv layers in the middle 8 layers if they exist
...
training_args = TrainingArguments(
    output_dir="./results_lfm_unmasked_lora_middle8layers",
    eval_strategy="epoch",
    learning_rate=2e-4,
...
print("Starting training for Custom LFM NER (Unmasked Middle 8 Layers + LoRA)...")
trainer.train()
...
with open("lfm_unmasked_middle_test_results.json", "w") as f:
