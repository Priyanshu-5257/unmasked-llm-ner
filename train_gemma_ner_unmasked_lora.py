...
def patched_gemma_model_forward(
    self,
    input_ids=None,
    attention_mask=None,
    position_ids=None,
    ...
    **kwargs,
    ...
    from transformers.models.gemma3.modeling_gemma3 import create_causal_mask, create_sliding_window_causal_mask, _bidirectional_window_overlay
    ...
    # Create non-causal mask (for unmasked layers)
    ...
    for i, decoder_layer in enumerate(self.layers[:num_layers]):
        ...
        if i >= num_layers - 8:
            layer_mask = expanded_non_causal_mask
        else:
...
class GemmaForTokenClassification(nn.Module):
    def __init__(self, model_name, num_labels):
        super().__init__()
        self.base_model = AutoModel.from_pretrained(model_name, trust_remote_code=True)
        self.base_model.forward = types.MethodType(patched_gemma_model_forward, self.base_model)
        ...
        self.classifier = SwiGLUClassifier(hidden_size, num_labels).to(self.base_model.dtype)
...
model = GemmaForTokenClassification(model_name, num_labels)
...
training_args = TrainingArguments(
    output_dir="./results_gemma_fewnerd_unmasked_lora_8layers",
    eval_strategy="epoch",
    learning_rate=2e-4,
...
print("Starting training for Gemma Few-NERD (Unmasked Last 8 Layers + LoRA)...")
trainer.train()
...
with open("gemma_fewnerd_unmasked_lora_test_results.json", "w") as f:
