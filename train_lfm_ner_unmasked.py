...
class LFMForTokenClassification(nn.Module):
    def __init__(self, model_name, num_labels):
        ...
        # Patch the conv layers in the last 2 layers if they exist
        ...
        # Freeze base model, except last 2 layers
        for name, param in self.base_model.named_parameters():
            if f"layers.{num_layers - 2}" in name or f"layers.{num_layers - 1}" in name:
...
training_args = TrainingArguments(
    output_dir="./results_lfm_unmasked",
    eval_strategy="epoch",
    learning_rate=2e-4,
...
print("Starting training for Custom LFM NER (Unmasked Last 2 Layers)...")
