...
class LFMForTokenClassification(nn.Module):
    def __init__(self, model_name, num_labels):
        ...
        # Custom head with CRF
        self.emissions = nn.Sequential(
        ...
        # Ensure the emissions and CRF require gradients
        for param in self.emissions.parameters():
    ...
    def forward(self, input_ids, attention_mask=None, labels=None, **kwargs):
        ...
        emissions = self.emissions(sequence_output)
        
        loss = None
        if labels is not None:
            labels_mask = (labels != -100)
            labels_clean = labels.clone()
            labels_clean[~labels_mask] = 0
            labels_mask[:, 0] = True
            labels_clean[:, 0] = 0
            loss = -self.crf(emissions, labels_clean, mask=labels_mask, reduction='mean')
            logits = emissions
        else:
...
training_args = TrainingArguments(
    output_dir="./results_lfm_unmasked_lora_8layers_crf",
    eval_strategy="epoch",
    learning_rate=2e-4,
...
print("Starting training for Custom LFM NER (Unmasked Last 8 Layers + LoRA + CRF)...")
trainer.train()
...
with open("lfm_unmasked_lora_crf_test_results.json", "w") as f:
