...
class LFMForTokenClassification(nn.Module):
    def __init__(self, model_name, num_labels):
        ...
        # Patch the conv layers in the last 4 layers if they exist
        ...
        # Freeze base model, except last 4 layers
        for name, param in self.base_model.named_parameters():
            # Check if name contains any layer index from num_layers-4 to num_layers-1
            unfrozen = any(f"layers.{i}" in name for i in range(num_layers - 4, num_layers))
            if unfrozen:
...
training_args = TrainingArguments(
    output_dir="./results_lfm_unmasked_4layers",
    eval_strategy="epoch",
    learning_rate=2e-4,
...
print("Starting training for Custom LFM NER (Unmasked Last 4 Layers)...")
trainer.train()
